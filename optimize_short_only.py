...
def main():
    ...
    # Short Thresholds to test: prob < 0.50 means bear probability > 0.50
    thresholds = [0.50, 0.45, 0.40, 0.35, 0.30, 0.25, 0.20, 0.15, 0.10, 0.05, 0.01, 0.005, 0.001]
    
    results = []
    print("\n" + "="*85)
    print(f"{'Short Threshold':<16} | {'Confidence':<12} | {'ROI':<9} | {'Sharpe':<8} | {'Trades':<8} | {'Shorts':<8} | {'Win/Loss':<11}")
    print("="*85)

    for threshold in thresholds:
        res = backtest_with_threshold(signals, close, high, low, seq_len, threshold)
        
        conf_str = f"{(1-threshold)*100:.1f}% BEAR"
    ...
    print("="*85)
    print(f"\nBest Performing Parameter: Prob < {best_thresh['threshold']} (ROI: {best_thresh['roi']:.2f}%)")
