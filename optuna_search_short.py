...
STATE_FILE = 'optuna_state_short.json'
LOG_FILE = 'optuna_output_short.log'
...
def create_objective(full_df, processed_path, train_end, val_end, device):
    ...
    def objective(trial):
        ...
        tp = trial.suggest_float('take_profit', 0.005, 0.04, step=0.0025)
        sl = trial.suggest_float('stop_loss', 0.005, 0.04, step=0.0025)
        ...
        if avg_roi >= 5.0: # Note: Lowered minimum validation threshold for shorts just in case
            ...
            if test_roi > 0 and test_trades >= 10:
                ...
                # Notify
                try:
                    msg = f"Optuna Short Trial #{trial.number}: TP={tp*100:.2f}% SL={sl*100:.2f}% | Val={avg_roi:.1f}% Test={test_roi:.1f}% Sharpe={test_sharpe:.2f}"
