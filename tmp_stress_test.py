...
print(f"{'MONTH':<10} | {'NORMAL (ALTs) ROI':<18} | {'PROP 22.88% ROI':<18}")
print("-" * 52)
...
for chunk_idx in range(total_chunks):
    ...
    print(f"Month {-month_id:<5} | {pnl_alts:>+7.2f}% ({trades_alts:>3}t) | {pnl_prop:>+7.2f}% ({trades_prop:>3}t)")
    
    total_pnl_prop += pnl_prop
    total_trades_prop += trades_prop
    total_pnl_alts += pnl_alts
    total_trades_alts += trades_alts

print("-" * 52)
print(f"TOTAL      | {total_pnl_alts:>+7.2f}% ({total_trades_alts:>4}t) | {total_pnl_prop:>+7.2f}% ({total_trades_prop:>4}t)")
