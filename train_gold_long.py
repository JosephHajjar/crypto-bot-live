...
MODEL_DIR = 'models_gold_long'
STATE_FILE = 'optuna_gold_long_state.json'
LOG_FILE = 'optuna_gold_long.log'

# Triple barrier params (must match feature_engineer.py)
TAKE_PROFIT = 0.0125   # 1.5%
STOP_LOSS = 0.006     # 0.75%
    ...
    def objective(trial):
        ...
        if avg_roi >= 12.3:
            ...
            if test_roi > 0 and test_trades >= 10:
                ...
                # Notify
                try:
                    msg = f"GOLD Optuna Trial #{trial.number}: Val={avg_roi:.1f}% Test={test_roi:.1f}% Sharpe={test_sharpe:.2f}"
...
def main():
    ...
    study = optuna.create_study(
        ...
        storage='sqlite:///optuna_gold_long.db',
        load_if_exists=True,
