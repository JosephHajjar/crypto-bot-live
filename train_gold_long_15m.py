...
from data.feature_engineer import engineer_features, get_feature_cols, _triple_barrier_label
...
MODEL_DIR = 'models_gold_long_15m'
STATE_FILE = 'optuna_gold_long_15m_state.json'
LOG_FILE = 'optuna_gold_long_15m.log'
...
# ================================================================
def backtest_triple_barrier_long(df_slice, model, seq_length, device, 
                            tp, sl, max_bars, fee_pct=FEE_PCT):
    """
    Backtest using the Triple Barrier method tailored for Longs.
    When model predicts 1 (enter trade):
      - Enter LONG at current close
      - Exit when TP (up) or SL (down) is hit, or after max_bars
    ...
    with torch.no_grad():
        for i in range(0, len(seq_tensor), 2048):
            ...
            bull_probs = probs[:, 1].cpu().tolist()  # Target 1 is long success
...
def walk_forward_evaluate(df, model, seq_length, device, tp, sl, max_bars, num_folds=NUM_FOLDS):
    ...
    for fold in range(num_folds):
        ...
        result = backtest_triple_barrier_long(df_fold, model, seq_length, device, tp=tp, sl=sl, max_bars=max_bars)
...
def create_objective(full_df, processed_path, train_end, val_end, device):
    ...
    def objective(trial):
        # Fixed targets (not hyperoptimized)
        tp = 0.005
        sl = 0.002
        max_bars = 16
        ...
        new_labels = _triple_barrier_label(full_df, take_profit=tp, stop_loss=sl, max_hold_bars=max_bars)
        ...
        if avg_roi >= 5.0:
            ...
            test_result = backtest_triple_barrier_long(df_test, model, seq_len, device, tp=tp, sl=sl, max_bars=max_bars)
            ...
            if test_roi > 0 and test_trades >= 10:
                ...
                print(f"  ** LONG MODEL SAVED: {model_path} **")
                
                # Notify
                try:
                    msg = f"GOLD Optuna 15m Long Trial #{trial.number}: TP={tp*100:.2f}% SL={sl*100:.2f}% | Val={avg_roi:.1f}% Test={test_roi:.1f}% Sharpe={test_sharpe:.2f}"
                ...
                if test_roi > TARGET_ANNUAL_ROI and test_sharpe > 1.0 and test_trades >= MIN_TRADES:
                    print("\n" + "=" * 60)
                    print("*** HOLY GRAIL LONG 15M FOUND! ***")
...
def main():
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name='trading_bot_long_15m',
        storage='sqlite:///optuna_gold_long_15m.db',
        load_if_exists=True,
